...
# if issues here then use "from skimage.feature.texture import greycomatrix" instead, apparently spelling changed
# also function calls were substituted below, but not in the comments
from Quality_Metrics.utils import bin_img, crop_img
...
def coent2d(img, brainmask = None, n_levels = 128, bin = True, crop = True, supress_zero = True):
    ...
    for slice in range(vol_shape[2]):
        tmp_co_oc_mat = graycomatrix(img[:,:,slice],
                                 distances = [1],
                                 angles = [0*(np.pi/2),
                                           1*(np.pi/2),
                                           2*(np.pi/2),
